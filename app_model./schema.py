...
def migrate_datasets_metadata(conn: Engine, csv_path: str = "DATA/datasets_metadata.csv") -> None:
    ...
    """
    # Load metadata from CSV into DataFrame
    ...
    try:
        data.to_sql(
            "datasets_metadata",
            conn,
            if_exists="replace",    # Avoid duplicate rows during development
            index=False             # Do not include pandas index in SQL table
        )
...
def get_all_datasets_metadata(conn: Engine) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: A DataFrame containing all dataset metadata rows.
